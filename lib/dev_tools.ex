defmodule Quillex.DevTools do
    ...
    with {:ok, viewport} <- get_viewport(viewport_name) do
      ...
      if buffers == [] do
        ...
        |> Enum.each(fn {buffer, idx} ->
          ...
          short_id =
            if is_binary(buffer_id) && String.length(buffer_id) > 8,
              do: String.slice(buffer_id, 0, 8) <> "...",
              else: to_string(buffer_id)

          # Show file path if available
          file_info =
            if buffer.semantic[:file_path] do
  ...
  defp find_buffer_by_id(entries, id) when is_binary(id) do
    ...
    result =
      Enum.find_value(entries, fn {_graph_key, data} ->
        data.elements
        |> Map.values()
        |> Enum.find(fn elem ->
  ...
  defp seems_like_code?(content) do
    ...
    patterns = [
      # Elixir
      ~r/\b(def|defp|defmodule|if|else|end)\b/,
      # JavaScript
      ~r/\b(function|const|let|var|return)\b/,
      # Python
      ~r/\b(class|def|import|from|return)\b/,
      # Common code syntax
      ~r/[{}();]/,
      # Indentation
      ~r/^\s{2,}/m
