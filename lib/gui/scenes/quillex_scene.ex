defmodule QuillEx.Scene.RootScene do
  ...
  def handle_cast(
    ...
    IO.puts("ROOT SCENE HANDLING ACTIONS #{inspect(actions)}")

    # new_scene =
    #   Enum.reduce(actions, scene, fn action, scene_acc ->
    #     QuillexReducer.process(scene_acc, action)
    #   end)
  ...
  #
  # all that to say, a gui_action is no different from any other kind
  # of action, except that it is generated by a GUI component, and might
  # have metadata e.g. a specific buffer to apply the action to
  def handle_cast(
    ...
    # we should just forward these to the buffer process actually
    BufferManager.cast_to_buffer(%{uuid: buf_uuid}, {:action, actions})
