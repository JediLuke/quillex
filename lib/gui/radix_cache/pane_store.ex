defmodule Quillex.RadixCache.PaneStore do
  ...
  # A buffer nobody has looked at yet has no remembered viewport; `viewed?:
  # false` lets the pane bring the cursor on screen instead of restoring an
  # origin that was never chosen (opening a file AT a search match).
  defp pane_snapshot(buf, state, uuid) do
    Map.put(
      buf,
      :pane_view,
      Map.get(state.views, uuid, %{offset_x: 0, offset_y: 0, folds: [], viewed?: false})
    )
  end
