  ...
  spex "Results open into one reusable preview tab",
    ...
    scenario "a result opens a preview, the next one replaces it, promotion keeps it" do
      ...
      then_ "double-clicking the tab promotes it, and the next result opens beside it",
        ...
        # Promotion must survive the buffer-list snapshot generated by the
        # tab activation click. An immediate next click can otherwise outrun
        # the race and make a broken local-only promotion look correct.
        Process.sleep(600)
        assert root_state().preview_buf_uuid == nil
        ...
        assert Enum.any?(root_state().buffers, &(&1.uuid == context.beta_uuid)),
               "the promoted tab itself must remain after another result opens"
