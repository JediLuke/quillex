defmodule Quillex.Property.TextEditorPropertiesTest do
  ...
  alias Quillex.Buffer.Core.{Document, Navigation, Selection}

  describe "cursor position invariants" do
    property "cursor is never positioned before the start of a line" do
      check all(
              buffer <- buffer_generator(),
              operations <- list_of(cursor_operation_generator(), max_length: 20)
            ) do
        final_buffer = Enum.reduce(operations, buffer, &apply_operation/2)

        cursor = final_buffer.cursor
    ...
    property "cursor on empty line is positioned at column 1" do
      check all(
              buffer <- buffer_generator(),
              operations <- list_of(text_operation_generator(), max_length: 10)
            ) do
        final_buffer = Enum.reduce(operations, buffer, &apply_operation/2)
  ...
  describe "text editing invariants" do
    property "buffer always contains at least one line" do
      check all(
              buffer <- buffer_generator(),
              operations <- list_of(text_operation_generator(), max_length: 20)
            ) do
        final_buffer = Enum.reduce(operations, buffer, &apply_operation/2)
    ...
    property "text insertion preserves existing content" do
      check all(
        ...
        {new_buffer, inserted} =
          try do
            {Document.insert_text(buffer, {cursor.line, cursor.col}, text), true}
          rescue
            _ -> {buffer, false}
          end
  ...
  describe "selection invariants" do
    property "selection boundaries are within document bounds" do
      check all(
              buffer <- buffer_generator(),
              operations <- list_of(selection_operation_generator(), max_length: 10)
            ) do
        final_buffer = Enum.reduce(operations, buffer, &apply_operation/2)
    ...
    property "replacing selection preserves document structure" do
      check all(
              buffer <- buffer_with_selection_generator(),
              replacement_text <- text_generator()
            ) do
          ...
          case buffer.selection do
            ...
            _selection ->
              try do
                ...
                Selection.delete_selected_text(buffer)
                |> then(fn buf ->
                  cursor = buf.cursor
                  Document.insert_text(buf, {cursor.line, cursor.col}, replacement_text)
                end)
              rescue
                _ -> buffer
              end
  ...
  defp buffer_generator do
    gen all(
          lines <- list_of(line_generator(), min_length: 1, max_length: 10),
          cursor_line <- integer(1..length(lines)),
          cursor_col <- cursor_col_generator(Enum.at(lines, cursor_line - 1, ""))
        ) do
      %BufState{
        data: lines,
  ...
  defp selection_operation_generator do
    one_of([
      gen all(
            direction <-
              one_of([constant(:up), constant(:down), constant(:left), constant(:right)]),
            count <- integer(1..3)
          ) do
  ...
  defp apply_operation({:move_cursor, direction, count}, buffer) do
    try do
      Navigation.move_cursor(buffer, direction, count)
    rescue
      # If operation fails, return original buffer
      _ -> buffer
    end
  end
  ...
  defp apply_operation({:move_cursor, direction}, buffer) do
    try do
      Navigation.move_cursor(buffer, direction)
    rescue
      _ -> buffer
    end
  end
  ...
  defp apply_operation({:insert_text, text}, buffer) do
    try do
      cursor = buffer.cursor
      Document.insert_text(buffer, {cursor.line, cursor.col}, text)
    rescue
      _ -> buffer
    end
  end
  ...
  defp apply_operation({:delete}, buffer) do
    try do
      cursor = buffer.cursor
      Document.delete_char_after_cursor(buffer, cursor)
    rescue
      _ -> buffer
    end
  end
  ...
  defp apply_operation({:newline}, buffer) do
    try do
      Document.insert_new_line(buffer, :at_cursor)
    rescue
      _ -> buffer
    end
  end
  ...
  defp apply_operation({:select_text, direction, count}, buffer) do
    try do
      Selection.select_text(buffer, direction, count)
    rescue
      _ -> buffer
    end
  end
  ...
  defp safe_apply_operation({:backspace}, buffer) do
    try do
      cursor = buffer.cursor
      Document.delete_char_before_cursor(buffer, cursor)
    rescue
      _ -> buffer
    end
  end
