defmodule QuillEx.Reducers.RadixReducer do
   @moduledoc """
   ...
   def change_font(%{editor: %{font: current_font}} = radix_state, new_font) when is_atom(new_font) do
   ...
   def change_font_size(%{editor: %{font: current_font}} = radix_state, direction) when direction in [:increase, :decrease] do
   ...
   def change_editor_scroll_state(radix_state, %{inner: %{width: _w, height: _h}, frame: _f} = new_scroll_state) do
      radix_state
